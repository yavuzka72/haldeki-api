...
class ProductController extends Controller {
      ...
      public function storefull(Request $request)
    {
        $data = $request->validate([
            ...
            // fiyatı yazacak kullanıcı (opsiyonel)
        ...
        return DB::transaction(function () use ($data, $request) {
            ...
            // FİYATI YAZACAK USER
            $priceOwner = $request->user();
            if (!$priceOwner && !empty($data['email'])) {
                $priceOwner = User::where('email', $data['email'])->first();
            }

            // VARYANTLAR (+ opsiyonel fiyat)
            foreach (($data['variants'] ?? []) as $v) {
                $incomingSku = isset($v['sku']) ? trim((string)$v['sku']) : '';
                $sku = $incomingSku !== ''
                    ? $this->ensureUniqueSku(Str::upper($incomingSku))
                    : $this->generateUniqueSku($data['name'], $v['name'] ?? 'varyant');
    ...
    {
        $base = Str::upper(Str::slug($productName . '-' . $variantName, '-'));
        if ($base === '') {
            $base = 'SKU';
        }

        return $this->ensureUniqueSku($base);
    }

    private function ensureUniqueSku(string $base): string
    {
        $candidate = $base;
        $i = 1;
  ...
  public function storefullOld(Request $request)
    {
        $data = $request->validate([
            'name'           => ['required','string','max:255'],
            'description'    => ['nullable','string'],
...
public function index(Request $request)
{
	...
	public function variantsUserMail(Request $request, Product $product)
{
    if (! $product->active) {
        return $this->error('Ürün bulunamadı.', 404);
    }
...
public function variantsUser(Request $request, Product $product)
{
    ...
    // 1) user_id: önce auth, yoksa email
    $userId = (int) optional($request->user())->id;

    if ($userId <= 0) {
        $email = trim((string) $request->query('email', ''));
        if ($email === '') {
            return $this->error('Unauthorized (token) veya email gerekli.', 422);
        }

        $userId = (int) User::where('email', $email)->value('id');
        if ($userId <= 0) {
            return $this->error('Kullanıcı bulunamadı.', 404);
        }
    }

    // 2) Varyantları çek + sadece bu user'ın son fiyatı (asla global latest değil)
    $variants = $product->variants()
        ->where('active', true)
        ->addSelect([
            'user_price' => UPP::select('price')
                ->whereColumn('product_variant_id', 'product_variants.id')
                ->where('user_id', $userId)     // ✅ şart
                ->latest('id')
                ->limit(1),

            // istersen tedarikçiye özel görsel:
            // 'user_image' => UPP::select('image')
            //     ->whereColumn('product_variant_id', 'product_variants.id')
        ...
        ->with(['prices' => function ($q) use ($userId) {
            // ✅ sadece bu kullanıcı
            $q->where('user_id', $userId)->orderByDesc('id');
        }])
        ->get();

    // 3) Artık extra "güvence" bloğuna gerek yok (subquery zaten user_id şartlı)
    // ama istersen 0/null normalize edebilirsin:
    $variants->each(function ($v) {
        if (is_null($v->user_price)) $v->user_price = 0;
    });

    return $this->success($variants);
}
 
public function variantsDealer(Request $request, Product $product)
{
    if (! $product->active) {
        return $this->error('Ürün bulunamadı.', 404);
    }

    // 1) dealer_id: önce auth kullanıcısından, yoksa query param
    $dealerId = (int) optional($request->user())->dealer_id;
    if ($dealerId <= 0) {
        $dealerId = (int) $request->query('dealer_id', 0);
    }
    if ($dealerId <= 0) {
        return $this->error('dealer_id gerekli (token veya query ile).', 422);
    }

    // 2) Varyantları çek + sadece bu dealer'ın son fiyatı
    $variants = $product->variants()
        ->where('active', true)
        ->addSelect([
            // ✅ bayi bazlı "son fiyat"
            'dealer_price' => UPP::select('price')
                ->whereColumn('product_variant_id', 'product_variants.id')
                ->where('dealer_id', $dealerId)
                ->orderByDesc('id')
                ->limit(1),

            // istersen bayi bazlı son tarih/id de al:
            'dealer_price_id' => UPP::select('id')
                ->whereColumn('product_variant_id', 'product_variants.id')
                ->where('dealer_id', $dealerId)
                ->orderByDesc('id')
                ->limit(1),
        ])
        ->with(['prices' => function ($q) use ($dealerId) {
            // ✅ sadece bu bayinin fiyat geçmişi
            $q->where('dealer_id', $dealerId)->orderByDesc('id');
        }])
        ->get();

    // 3) null -> 0
    $variants->each(function ($v) {
        $v->dealer_price = (float) ($v->dealer_price ?? 0);
    });
