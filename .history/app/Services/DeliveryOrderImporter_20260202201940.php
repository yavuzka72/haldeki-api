...
*/
            ...
            $pc = $o->relationLoaded('partnerClient')
                ? $o->partnerClient
                : ($o->partner_client_id ? $o->partnerClient()->first() : null);

            $pickupName  = optional($pc)->name ?: ($dealerName ?: 'Depo');
            $pickupAddr  = optional($pc)->address ?: ($dealerAddr ?: '—');
            $pickupPhone = optional($pc)->phone ?? $dealerPhone;

            $pickupLat   = optional($pc)->latitude  ?? optional($dealer)->latitude;
            $pickupLong  = optional($pc)->longitude ?? optional($dealer)->longitude;

            $pickup = [
            'name'          => $pickupName,
            'address'       => $pickupAddr,
            'city'          => optional($pc)->city ?? optional($dealer)->city,
            'district'      => optional($pc)->district ?? optional($dealer)->district,
