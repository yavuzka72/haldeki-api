 ...
 public function index(Request $request)
    {
        ...
        if ($ownerUserId <= 0) {
            return response()->json([
                'message' => 'Partner is missing user_id mapping (partner_clients.user_id).',
            ], 422);
        }
        ...
        $productsQ = DB::table('products as p')
            ->select([
                'p.id', 'p.name', 'p.slug', 'p.image', 'p.description', 'p.is_active', 'p.updated_at',
            ]);

        if ($onlyActive === 1) {
            $productsQ->where('p.is_active', 1);
        }
