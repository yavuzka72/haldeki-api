  ...
  public function buildPriceList(Request $request)
    {
        ...
        $partner = DB::table('partner_clients')
            ->where('partner_key', $key)
            ->where('partner_secret', $secret)
            ->where('api_token', $token)   // sende alan adı farklıysa değiştir
            ->where('is_active', 1)
            ->first();
        ...
        // --- Payload ---
        $oran = (float) $request->input('multiplier', 1.0);
        $roundMultiplier = (int) $request->input('round_multiplier', 3);
        $roundPrice      = (int) $request->input('round_price', 2);

        if ($oran <= 0) {
            return response()->json(['message' => 'multiplier invalid'], 422);
        }

        // supplier_user_id: fiyat bazını hangi user’dan alacağız?
        // en doğrusu partner_clients tablosunda bir user_id/supplier_user_id tutmak:
        $supplierUserId = (int) ($partner->supplier_user_id ?? $partner->user_id ?? 0);
        if ($supplierUserId <= 0) {
            return response()->json(['message' => 'supplier_user_id missing on partner'], 422);
        }

        return DB::transaction(function () use ($partnerClientId, $oran, $supplierUserId, $roundMultiplier, $roundPrice) {

            // 1) BASE -> PARTNER VARIANT INSERT (NOT EXISTS)
            // base: partner_client_id NULL
            // uniq: (partner_client_id, product_id, sku)
            $baseVariants = DB::table('product_variants as v')
                ->select('v.product_id', 'v.name', 'v.sku', 'v.unit', 'v.multiplier')
                ->whereNull('v.partner_client_id');

            // Insert...Select with NOT EXISTS
            $insertSql = "
                INSERT INTO product_variants
                    (partner_client_id, product_id, name, sku, unit, multiplier, active, created_at, updated_at)
                SELECT
                    ?, v.product_id, v.name, v.sku, v.unit, ROUND(v.multiplier * ?, ?), 1, NOW(), NOW()
            ...
            DB::statement($insertSql, [$partnerClientId, $oran, $roundMultiplier, $partnerClientId]);
            ...
            $updateMultiplierSql = "
                ...
                JOIN product_variants bv
                 ...
                 AND bv.sku = pv.sku
                SET pv.multiplier = ROUND(bv.multiplier * ?, ?),
                    pv.updated_at = NOW()
                WHERE pv.partner_client_id = ?
            ";
            DB::statement($updateMultiplierSql, [$oran, $roundMultiplier, $partnerClientId]);

            // 3) VARSA PARTNER PRICE UPDATE
            $updatePricesSql = "
                ...
                WHERE pp.partner_client_id = ?
                  AND pp.user_id = ?
            ";
            DB::statement($updatePricesSql, [
                $partnerClientId, $supplierUserId, $oran, $roundPrice, $partnerClientId, $supplierUserId
            ]);

            // 4) EKSİK PARTNER FİYATLARI INSERT
            $insertPricesSql = "
                INSERT INTO user_product_prices
                    (partner_client_id, user_id, product_variant_id, price, active, created_at, updated_at)
                SELECT
                    ?, ?, pv.id, ROUND(bp.price * ?, ?), 1, NOW(), NOW()
                FROM product_variants pv
                JOIN product_variants bv
                  ON bv.partner_client_id IS NULL
                 AND bv.product_id = pv.product_id
                ...
                WHERE pv.partner_client_id = ?
                  AND pp.id IS NULL
            ";
            DB::statement($insertPricesSql, [
                $partnerClientId, $supplierUserId, $oran, $roundPrice,
                $supplierUserId, $partnerClientId, $supplierUserId, $partnerClientId
            ]);
