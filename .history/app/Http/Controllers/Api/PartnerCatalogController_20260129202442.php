...
public function indexallprice(Request $request)
    ...
    public function buildPriceList(Request $request)
    {
        ...
        |--------------------------------------------------------------------------
        */
        $partnerKey    = (string) $request->header('X-Partner-Key', '');
        $partnerSecret = (string) $request->header('X-Partner-Secret', '');
        $tokenHeader   = (string) $request->header('Authorization', '');
        ...
        | 2) PARTNER DOĞRULAMA
        |--------------------------------------------------------------------------
        | partner_clients tablonu varsayıyorum
        */
        $partner = DB::table('partner_clients')
        ...
        // opsiyonel token kontrolü
        if (!empty($partner->token) && $tokenHeader) {
            $bearer = str_replace('Bearer ', '', $tokenHeader);
            if ($bearer !== $partner->token) {
                return response()->json([
                    'success' => false,
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | 4) ANA VARIANTLAR (partner_client_id IS NULL)
        |--------------------------------------------------------------------------
        */
        $baseVariants = DB::table('product_variants')
            ->whereNull('partner_client_id')   // ANA fiyat
        ...
        foreach ($baseVariants as $baseVariant) {
            ...
            $basePrice = DB::table('user_product_prices')
                ->where('product_variant_id', $baseVariant->id)
            ...
            $newPrice = round($basePrice * $multiplier, $round);

            /*
            |--------------------------------------------------------------------------
            | 5) PARTNER VARIANT (UPSERT)
            |--------------------------------------------------------------------------
            */
            $partnerVariant = DB::table('product_variants')
                ->where('product_id', $baseVariant->product_id)
                ->where('name', $baseVariant->name)
            ...
            if (!$partnerVariant) {
                ...
                DB::table('user_product_prices')->insert([
                ...
                DB::table('user_product_prices')
                    ->updateOrInsert(
                        [
                            'product_variant_id' => $partnerVariant->id,
                            'partner_client_id'  => $partnerClientId,
                        ],
                        [
                            'price'      => $newPrice,
