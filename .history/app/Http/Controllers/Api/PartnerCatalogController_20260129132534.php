  ...
  public function index(Request $request)
    {
        ...
        // ✅ Partner hangi kullanıcının (owner) fiyatlarını görecek?
        // partner_clients tablosunda user_id olmalı
        ...
        // Opsiyonel arama (ürün/variant içinde)
        ...
        if ($q !== '') {
            // ürün isminden filtreledik ama ayrıca variant adına göre de kısıtlamak istersen:
            // Not: Bu ürün sayfasında bazı ürünler gelip variant boş kalabilir.
            // İstersen bunu kapatabilirsin.
            $variantsQ->where('v.name', 'like', '%' . $q . '%');
        }
        ...
        // --- Prices (partner_id’ye göre!) ---
        // Öncelik:
        // 1) upp.partner_client_id = partner->id (override)
        // 2) yoksa upp.partner_client_id IS NULL ve upp.user_id = partner->user_id (fallback)
        $priceByVariant = [];

        if (!empty($variantIds)) {
            // 1) Partner override fiyatları (✅ partner_id’ye göre filtre!)
            ...
            // 2) Owner user fiyatları (fallback)
            if ($ownerUserId > 0) {
