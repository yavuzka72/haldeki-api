  ...
  public function buildPriceList(Request $request)
    {
        ...
        $partner = DB::table('partner_clients')
            ->where('partner_key', $key)
            ->where('partner_secret', $secret)
            ->where('api_token', $token) // sende token alanı farklıysa değiştir
            ->where('is_active', 1)
            ->first();
        ...
        // supplier_user_id: base fiyatı hangi user'dan alacağız?
        // (En doğrusu partner_clients tablosunda supplier_user_id tutmak)
        $supplierUserId = (int) ($partner->supplier_user_id ?? $partner->user_id ?? 1);

        return DB::transaction(function () use ($partnerClientId, $oran, $round, $supplierUserId) {

            // 1) BASE VARIANT → PARTNER VARIANT OLUŞTUR (INSERT ... SELECT ... NOT EXISTS)
            // Not: base variants partner_client_id IS NULL varsayımı
            $insertSql = "
            ...
            ";
            $variantsInserted = DB::affectingStatement($insertSql, [
                $partnerClientId,
                $oran,
                $partnerClientId,
            ]);

            // 2) MULTIPLIER GÜNCELLE (UPDATE ... JOIN ...)
            $updateMultSql = "
                ...
                WHERE pv.partner_client_id = ?
            ";
            $variantsUpdated = DB::affectingStatement($updateMultSql, [
                $oran,
                $partnerClientId,
            ]);

            // 3) VARSA PARTNER FİYATI GÜNCELLE
            $updatePricesSql = "
                ...
                WHERE pp.partner_client_id = ?
                  AND pp.user_id = ?
            ";
            $pricesUpdated = DB::affectingStatement($updatePricesSql, [
                $partnerClientId,
                $supplierUserId,
                $oran,
                $round,
            ...
            // 4) EKSİK PARTNER FİYATLARINI INSERT ET
            $insertPricesSql = "
                ...
                WHERE pv.partner_client_id = ?
                  AND pp.id IS NULL
            ";
            $pricesInserted = DB::affectingStatement($insertPricesSql, [
                $partnerClientId,
                $supplierUserId,
                $oran,
                $round,
