 ...
 public function buildPriceList(Request $request)
    {
        ...
        $key    = (string) $request->header('X-Partner-Key', '');
        $secret = (string) $request->header('X-Partner-Secret', '');
        $token  = (string) $request->bearerToken(); // opsiyonel
        ...
        // ✅ ARTIK partner_client_id BURADAN
        ...
        // --- BASE VARIANTLER ---
        // partner_client_id IS NULL → ana (master) varyantlar
        ...
        foreach ($variants as $baseVariant) {
            ...
            $baseMultiplier = (float) ($baseVariant->multiplier ?? 1);
            $newMultiplier  = round($baseMultiplier * $multiplier, 4);

            // Aynı ürün + aynı isim için partner varyant var mı?
