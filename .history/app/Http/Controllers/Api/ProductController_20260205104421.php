...
public function variantsDealer(Request $request, Product $product)
{
    ...
    if ($dealerId <= 0) {
        ...
        if ($dealerId <= 0) {
            return $this->error('Unauthorized (token) veya dealer_id gerekli.', 422);
        }
    }

    // 2) Varyantlar + sadece bu dealer'ın son fiyatı
    $variants = $product->variants()
        ->where('active', true)
        ->addSelect([
            'dealer_price' => UPP::select('price')
                ->whereColumn('product_variant_id', 'product_variants.id')
                ->where('dealer_id', $dealerId)   // ✅ dealer bazlı
                ->latest('id')
                ->limit(1),
        ])
        ->with(['prices' => function ($q) use ($dealerId) {
            $q->where('dealer_id', $dealerId)->orderByDesc('id'); // ✅ sadece o bayi
        }])
        ->get();

    // 3) normalize
    $variants->each(function ($v) {
        if (is_null($v->dealer_price)) $v->dealer_price = 0;
    });
