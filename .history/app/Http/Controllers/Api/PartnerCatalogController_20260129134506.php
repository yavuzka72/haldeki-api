...
public function index(Request $request)
{
    ...
    // ✅ Partner'ın bağlı olduğu owner user
    $ownerUserId = 1;// (int) ($partner->user_id ?? 0);

    // Eğer partner user_id tanımlı değilse fallback fiyat çalışmaz
    // ama override fiyatlar yine de çalışır.
    // İstersen burada 422 döndürebilirsin.
    // if ($ownerUserId <= 0) { ... }
    ...
    // --- Prices ---
    // Öncelik:
    // 1) partner_client_id = partner->id (override)
    // 2) yoksa user_id = partner->user_id AND partner_client_id IS NULL (default)
    $priceByVariant = [];

    if (!empty($variantIds)) {
        // ✅ Override fiyatlar (SADECE bu partner)
