 ...
 public function index(Request $request)
{
    ...
    // ✅ Partner’ın fiyat sahibi user_id’si (partner_clients tablosunda user_id olmalı)
    $ownerUserId = (int) ($partner->user_id ?? 0);
    ...
    // (Opsiyonel) Ürün adına göre arama
    ...
    if (empty($productIds)) {
        return response()->json([
            'partner' => ['id' => (int) $partner->id, 'name' => $partner->name ?? null, 'user_id' => $ownerUserId],
            'data' => [],
    ...
    // --- Variants ---
    $variantsQ = DB::table('product_variants as v')
        ->whereIn('v.product_id', $productIds)
        ->select(['v.id', 'v.product_id', 'v.name', 'v.sku', 'v.unit', 'v.multiplier', 'v.active', 'v.updated_at'])
        ->orderBy('v.product_id')
        ->orderBy('v.id');

    // ✅ Eğer product_variants tablosunda partner_client_id varsa:
    // Global (NULL) + Bu partner (partner->id)
    // Yoksa bu blok sorun çıkarmaz mı? Çıkarır. O yüzden kolonu gerçekten varsa kullan.
    // Kolon varsa aç:
    $variantsQ->where(function ($qq) use ($partner) {
        $qq->whereNull('v.partner_client_id')
           ->orWhere('v.partner_client_id', (int) $partner->id);
    });

    $variants = $variantsQ->get();
    ...
    // --- Prices ---
    // Öncelik:
    // 1) upp.partner_client_id = partner->id
    // 2) yoksa (NULL) + upp.user_id = ownerUserId
    $priceByVariant = [];

    if (!empty($variantIds)) {
        // ✅ Partner override fiyatlar (sadece bu partner)
        $overrideRows = DB::table('user_product_prices as upp')
            ->where('upp.active', 1)
            ->where('upp.partner_client_id', (int) $partner->id)
        ...
        // ✅ User fiyatları (fallback) (ownerUserId varsa)
        if ($ownerUserId > 0) {
            $userRows = DB::table('user_product_prices as upp')
                ->where('upp.active', 1)
                ->whereNull('upp.partner_client_id')
