...
public function indexDetayli(Request $request)
{
    // --- Partner Auth ---
    ...
    $dealerId  = (int) ($partner->dealer_id ?? 0);

    if ($dealerId <= 0) {
        return response()->json([
            'message' => 'Partner için dealer_id tanımlı değil.',
    ...
    // =========================================================
    // 1) Önce dealer'a ait varyantların product_id listesini bul
    // =========================================================
    ...
    // =========================
    // 2) Products (sadece allowed + dealer güvenliği)
    // =========================
    ...
    // ✅ Eğer products tablosunda dealer_id varsa kesin ekle (varsa çok iyi):
    if (Schema::hasColumn('products', 'dealer_id')) {
        $productsQ->where('p.dealer_id', $dealerId);
    }
    ...
    // =========================
    // 3) Variants (dealer filtresi)
    // =========================
    $variants = DB::table('product_variants as v')
        ->whereIn('v.product_id', $productIds)
        ->where('v.dealer_id', $dealerId) // ✅ dealer varyantları
    ...
    if (!empty($variantIds)) {
        $priceRows = DB::table('user_product_prices as upp')
            ->where('upp.active', 1)
            ->where('upp.dealer_id', $dealerId) // ✅ dealer’a özel fiyat
    ...
    foreach ($products as $p) {
        ...
        // ürünün dealer varyantı yoksa hiç gönderme
        if (empty($pVariants)) {
            continue;
        }
...
public function indexDealer(Request $request)
{
    // --- Partner Auth ---
    $key    = (string) $request->header('X-Partner-Key', '');
    $secret = (string) $request->header('X-Partner-Secret', '');
    ...
    public function buildPriceList(Request $request)
{
    ...
    $partnerClientId = (int) $partner->id; // sadece response için
    $dealerId        = (int) ($partner->dealer_id ?? 0);

    if ($dealerId <= 0) {
        return response()->json([
            'message' => 'partner_clients.dealer_id boş. Dealer bazlı fiyat için dealer_id zorunlu.'
        ], 422);
    }
    ...
    // Base fiyat hangi user_id’deyse
    $supplierUserId = 18; // (int) ($partner->supplier_user_id ?? 0);
    if ($supplierUserId <= 0) {
        return response()->json([
            'message' => 'Base fiyatları hangi user tutuyorsa onu yaz (örn: 18).'
        ], 422);
    }

    return DB::transaction(function () use ($partnerClientId, $dealerId, $oran, $round, $supplierUserId) {

        /**
         * BASE TANIMI:
         * - Base varyant: product_variants.dealer_id IS NULL  (ana katalog)
         * - Dealer varyant: product_variants.dealer_id = ?
         *
         * Eğer senin base varyantların başka bir dealer_id’de tutuluyorsa (örn dealer_id=1),
         * buradaki "IS NULL" kısmını o id ile değiştir.
         */

        // 1) BASE → DEALER VARIANT INSERT (yoksa)
        $variantsInserted = DB::affectingStatement("
            ...
            WHERE v.dealer_id IS NULL
              AND NOT EXISTS (
                ...
                WHERE dv.dealer_id = ?
                  AND dv.product_id = v.product_id
                  AND dv.sku = v.sku
              )
        ", [$dealerId, $oran, $dealerId]);

        // 2) DEALER VARIANT multiplier update (base’e göre)
        $variantsUpdated = DB::affectingStatement("
            UPDATE product_variants dv
            JOIN product_variants bv
              ON bv.dealer_id IS NULL
             AND bv.product_id = dv.product_id
             AND bv.sku = dv.sku
            SET dv.multiplier = ROUND(bv.multiplier * ?, 3),
                dv.updated_at = NOW()
            WHERE dv.dealer_id = ?
        ", [$oran, $dealerId]);

        // 3) VARSA dealer fiyatları güncelle (base price * oran)
        $pricesUpdated = DB::affectingStatement("
            UPDATE user_product_prices dp
            JOIN product_variants dv
              ON dv.id = dp.product_variant_id
             AND dv.dealer_id = ?
            JOIN product_variants bv
              ON bv.dealer_id IS NULL
             AND bv.product_id = dv.product_id
            ...
            JOIN user_product_prices bp
             ...
             AND bp.user_id = ?
             AND bp.dealer_id IS NULL
            SET dp.price = ROUND(bp.price * ?, ?),
                dp.updated_at = NOW()
            WHERE dp.dealer_id = ?
              AND dp.user_id = ?
        ", [$dealerId, $supplierUserId, $oran, $round, $dealerId, $supplierUserId]);

        // 4) EKSİK dealer fiyatlarını insert et
        $pricesInserted = DB::affectingStatement("
            ...
            JOIN user_product_prices bp
             ...
             AND bp.user_id = ?
             AND bp.dealer_id IS NULL
            LEFT JOIN user_product_prices dp
              ON dp.product_variant_id = dv.id
             AND dp.dealer_id = ?
             AND dp.user_id = ?
            WHERE dv.dealer_id = ?
              AND dp.id IS NULL
        ", [$dealerId, $supplierUserId, $oran, $round, $supplierUserId, $dealerId, $supplierUserId, $dealerId]);
...
public function buildPriceListPartner(Request $request)
{
    // --- Partner Auth ---
    $key    = (string) $request->header('X-Partner-Key', '');
    $secret = (string) $request->header('X-Partner-Secret', '');
