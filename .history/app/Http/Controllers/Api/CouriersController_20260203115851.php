...
class CouriersController extends Controller
{
  ...
  public function activeCouriers(Request $request)
    {
        // OpsMapScreen CourierLive.tryParse; latitude/location_lat destekliyor :contentReference[oaicite:7]{index=7}
        $rows = User::query()
            ->select('id','name','username','user_type','is_active','can_take_orders','dealer_id',
                     'latitude','longitude','location_lat','location_lng','updated_at')
            ->where('user_type', 'delivery_man')
            ->where('is_active', 1)
            ->where('can_take_orders', 1)
            ->where(function($q){
                $q->whereNotNull('latitude')->whereNotNull('longitude')
                  ->orWhere(function($q2){ $q2->whereNotNull('location_lat')->whereNotNull('location_lng'); });
            })
