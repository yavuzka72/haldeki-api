 ...
 public function buildPriceList(Request $request)
{
    ...
    if ($key === '' || $secret === '') {
        return response()->json(['message' => 'Missing partner credentials.'], 401);
    }
    ...
    $partner = DB::table('partner_clients')
        ->where('partner_key', $key)
        ->where('partner_secret', $secret)
        ->where('token', $token)     // sende kolon adı api_token vs ise değiştir
        ->where('is_active', 1)
        ->first();
    ...
    // Base fiyatları hangi tedarikçi user'dan alacağız?
    // En doğrusu partner_clients içinde supplier_user_id tutmak.
    $supplierUserId = (int) ($partner->supplier_user_id ?? 0);
    if ($supplierUserId <= 0) {
        return response()->json(['message' => 'supplier_user_id missing on partner_clients'], 422);
    }

    return DB::transaction(function () use ($partnerClientId, $oran, $round, $supplierUserId) {

        // 1) BASE VARIANT -> PARTNER VARIANT INSERT (Sadece partner_client_id IS NULL olanlardan)
        ...
        // 2) Partner variant multiplier'larını base'e göre güncelle
        ...
        // 3) Varsa partner price güncelle (hedef partner_client_id ile!)
        ...
        // 4) Eksik partner price'ları insert et
