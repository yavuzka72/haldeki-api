...
use App\Http\Controllers\Api\OpsMapController;
...
use App\Http\Controllers\Api\PartnerPriceSyncController;
...
Route::prefix('v1')->group(function () {
    ...
    Route::post('dealer-orders-pending', [OrderController::class, 'dealerPending']);
 



      // Ops map
    Route::post('dealer-delivery-orders', [OpsMapController::class, 'dealerDeliveryOrders']);
    Route::get('dealer-delivery-orders', [OpsMapController::class, 'dealerDeliveryOrdersGet']);
    Route::get('couriers/live', [OpsMapController::class, 'liveCouriers']);
    Route::post('delivery-orders/{deliveryOrder}/assign', [OpsMapController::class, 'assignCourier']);
    Route::get('active-couriers', [CouriersController::class, 'activeCouriers']);



        // Dealer delivery reports / bulk actions
    Route::post('dealer-orders-delivery', [DeliveryOrderController::class, 'dealerDeliver']);
    Route::post('dealer-order-detail-delivery', [DeliveryOrderController::class, 'orderDetailDealerDeliver']);
    Route::post('dealer-order-update-status-delivery', [DeliveryOrderController::class, 'dealerUpdateStatusDeliver']);
    Route::post('dealer-collections-report', [DeliveryOrderController::class, 'dealerCollectionsReport']);
    Route::post('bulk-charge', [DeliveryOrderController::class, 'bulkCharge']);
    Route::post('bulk-pay-courier', [DeliveryOrderController::class, 'bulkPayCourier']);

    // Customer lookup
    Route::get('customers/by-phone', [CustomerLookupController::class, 'byPhone']);
    ...
    Route::middleware('auth:sanctum')->group(function () {
       ...
       Route::get('products/{product}/variantsuser', [ProductController::class, 'variantsUser']);
           Route::post('user-product-prices/upsert', [VariantPriceController::class, 'upsert']);
             Route::get('/products/{product}/variants/dealer', [VariantPriceController::class, 'variantsDealer']);
...
Route::prefix('partner/v1')->group(function () {
    ...
    Route::middleware('partner.auth')->group(function () {
        ...
        Route::post('build-price-list', [PartnerCatalogController::class, 'buildPriceList']);
          // ✅ Değişen fiyatları getir (incremental)
        Route::get('price-changes', [PartnerPriceSyncController::class, 'changes']);

        // ✅ Sunucuda cursor oku (opsiyonel)
        Route::get('price-sync-state', [PartnerPriceSyncController::class, 'state']);

        // ✅ Sunucuda cursor güncelle (opsiyonel - “ack”)
        Route::post('price-sync-ack', [PartnerPriceSyncController::class, 'ack']);
